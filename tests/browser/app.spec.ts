...
test.describe('tag profile', () => {
  test.use({ timezoneId: 'UTC' })

  test('shares observed coverage and history with diagnostics and pauses replay across navigation', async ({ page }) => {
    await setup(page)
    await page.clock.pauseAt(new Date('2026-09-13T12:01:00Z'))
    let historyRequests = 0
    await page.route('**/positions/*', route => {
      historyRequests++
      return route.fulfill({ json: [0, 5, 20, 25].map(second => ({
        ...samples[0], ts: new Date(today.getTime() - 120000 + second * 1000).toISOString(),
      })) })
    })
    await page.getByRole('button', { name: 'Ficha del tag', exact: true }).click()
    await expect(page.getByRole('heading', { name: 'Ficha del tag' })).toBeFocused()
    await expect(page.getByRole('region', { name: 'Identidad del tag' })).toContainText('Sin nombre asignado')
    const openReplay = page.getByRole('button', { name: 'Ver recorrido en el plano' })
    await expect(openReplay).toBeDisabled()
    await page.getByLabel('Desde', { exact: true }).fill('2026-09-13T11:58')
    await page.getByLabel('Hasta', { exact: true }).fill('2026-09-13T11:59')
    await page.getByRole('button', { name: 'Cargar jornada', exact: true }).click()
    const coverage = page.getByRole('region', { name: 'Cobertura del periodo' })
    await expect(coverage).toContainText('16,67')
    await expect(page.getByRole('group', { name: 'Resumen del histórico' })).toContainText('10 s')
    await page.getByRole('button', { name: 'Ver diagnóstico de señal' }).click()
    ...
    const cursor = await page.getByRole('slider').inputValue()
    await page.getByRole('button', { name: 'Ficha del tag', exact: true }).click()
    await page.clock.runFor(1000)
    await openReplay.click()
    await expect(page.getByRole('button', { name: 'Reproducir jornada' })).toBeVisible()
    await expect(page.getByRole('slider')).toHaveValue(cursor)
    expect(historyRequests).toBe(1)
    await page.getByRole('button', { name: 'Ficha del tag', exact: true }).click()
    await page.getByRole('button', { name: /T1 T1/ }).click()
    await expect(coverage).toHaveCount(0)
    await expect(openReplay).toBeDisabled()
    await page.getByRole('button', { name: 'Ver en el plano' }).click()
    await expect(page.getByRole('heading', { name: 'Posiciones actuales' })).toBeFocused()
    await expect(page.getByRole('button', { name: /T1 T1/ })).toHaveAttribute('aria-pressed', 'true')
  })

  test('distinguishes missing catalogue, inactive tags, empty history and failed history', async ({ page }) => {
    await setup(page)
    await page.route('**/tags', route => route.fulfill({ json: [] }))
    await page.reload()
    await page.getByRole('button', { name: 'Ficha del tag', exact: true }).click()
    await expect(page.getByText('Todavía no hay un tag seleccionado.', { exact: false })).toBeVisible()
    await expect(page.getByRole('button', { name: 'Ver en el plano' })).toBeDisabled()
    await page.route('**/tags', route => route.fulfill({ json: [{ id: 'T0', employee: '  ', active: false }] }))
    await page.reload()
    await page.getByRole('button', { name: 'Ficha del tag', exact: true }).click()
    await expect(page.getByRole('region', { name: 'Identidad del tag' })).toContainText('Inactivo')
    await expect(page.getByRole('region', { name: 'Identidad del tag' })).toContainText('Sin nombre asignado')
    await page.route('**/positions/*', route => route.fulfill({ json: [] }))
    await page.getByRole('button', { name: 'Cargar jornada', exact: true }).click()
    await expect(page.getByRole('region', { name: 'Cobertura del periodo' })).toContainText('0')
    await expect(page.getByRole('region', { name: 'Cobertura del periodo' })).toContainText('Sin muestras')
    await expect(page.getByRole('button', { name: 'Ver recorrido en el plano' })).toBeDisabled()
    await page.route('**/positions/*', route => route.fulfill({ status: 503 }))
    await page.getByRole('button', { name: 'Cargar jornada', exact: true }).click()
    await expect(page.getByRole('alert')).toContainText('503')
    await expect(page.getByRole('region', { name: 'Cobertura del periodo' })).toHaveCount(0)
  })
})
